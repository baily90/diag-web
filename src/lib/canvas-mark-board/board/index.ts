...
export default class CanvasMarkBoard implements ICanvasMarkBoard {
  ...
  getDistance(p1, p2) {
    return Math.sqrt(Math.pow((p1.x - p2.x) * this.config.mm_per_pixel, 2) + Math.pow((p1.y - p2.y) * this.config.mm_per_pixel, 2))
  }
  /** TODO:优化全部渲染 */
  render() {
    ...
    this.ctx.font = `bold ${~~((16 / this.t.a) < 1 ? 1 : (16 / this.t.a))}px serif`;
  ...
  appMousemove(e: MouseEvent) {
    ...
    const drawStatus = !!this.markObjectList.find(
